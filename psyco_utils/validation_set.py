...
import pandas as pd
from NaturalMouseTracker import NaturalMouseTracker
import cv2
import os
...
def create_validation_Video(folder,config_path):
    ...
    dics={'bboxes':eval,'sort_tracks':eval,'RFID_tracks':eval,'sort_tracks_ori':eval,'Correction':eval,'RFID_matched':eval,'Matching_details':eval}#'bpts':eval}
    ...
    frame_count=0
    pbar=tqdm.tqdm(total=vid_length,position=0,leave=False)
    while vid.isOpened():
        ret,img=vid.read()
        if ret and frame_count<vid_length-1:   
            ...
            #blankimg=255*np.ones(shape=[height+200,3*width,3],dtype=np.uint8)
            ...
            #cv2.putText(blankimg,'RFID Tracking',(150+int(0.5*width),100),0, 5e-3 * 200,(0,0,255),3)
            #print(RFID_frame)
            if RFID_frame != [] and RFID_frame !=0:
                #print(RFID_readings[RFID_frame])
                spacer=0
                for i in RFID_readings[RFID_frame]:
                    if i[1] in tags:
                        ...
                        cv2.putText(blankimg,f'Frame {RFID_frame}: reader {str(reader_display)}    tag read {tag_display}', \
                                    (int(1.4*width),int(height+275+spacer)),0,5e-3 * 210,(0,0,255),3)#1.4*height+spacer
                        spacer+=50        
