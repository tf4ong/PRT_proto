...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), show_label=True):
    ...
    for i in range(num_boxes[0]):
        ...
        class_name = 'rodent'#classes[class_ind]
        ...
        bbox_list.append([coor[1], coor[0],coor[3], coor[2],score])
...
def motion_detection(frame,avg,min_area=75,intensity_thres=25, k_size=(9,9)):
    status='No_Motion'
    ...
    thresh = cv2.threshold(frameDelta, intensity_thres, 255, cv2.THRESH_BINARY)
    ...
    for c in cnts:
        if cv2.contourArea(c) < min_area:
            ...
            cnt_bb.append([x,y,x+w,y+h])
            #cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
            status='Motion'
    return avg,status, cnt_bb
