...
class VideoWrite:
    ...
    def __init__(self,output,fps,resolution,frame=None):
        self.frame = frame
        self.stopped = False
        codec = cv2.VideoWriter_fourcc(*'XVID')
        self.output= cv2.VideoWriter(output, codec, fps, (resolution[0], resolution[1]),True)
...
def validation_frame_gen(config_dic,tags,frame,frame_count,
    ...
    if  md_frame != [] and md_frame !=0:
        for i in match_details[md_frame]:
                ...
                spacer+=30
    return blankimg
def threadBoth(path,config_dic,tags,df1,validation_frames):
    """
    Dedicated thread for grabbing video frames with VideoGet object.
    Dedicated thread for writing video frames with VideoShow object.
    ...
    vid.release()
    video_writer = VideoWrite(path+'/val.avi',fps,[1920,1080])
    video_writer.start()
